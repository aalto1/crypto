...
def random_replicated_sharing(j, prfs, key):
    ...
    """
    return [(s, prf(key)) for (s, prf) in prfs.iteritems()] 
...
class PRF(object):
    ...
    def __init__(self, key, max):
        ...
        bit_length = (max-1).bit_length()

        # Number of whole digest blocks needed.
        blocks = (bit_length + 8*sha.digest_size-1) / (8*sha.digest_size)
        # Number of whole bytes needed.
        self.bytes = (bit_length + 7) / 8
    ...
    def __call__(self, input):
        ...
        be given if one wants to evaluate the PRF on more than 2**32
        different values.
